...
BLEU results from testing both stress datasets are shown in Figure 3. The results from the test set of the original dataset of the codebase used to train is also provided for reference. While the scores appear poor, this result is to be expected as the stress datasets contain more complex examples as mentioned earlier. An example of the normal dataset is given (of the German input, expected English translation, and generated translation from top to bottom):
...
\end{tabular}
\end{center}
In respect to the reference used to calculate the BLEU score, this sentence demonstrate decent humanness, equating to the relatively acceptable score for this model despite it being incorrectly translated as underlined.

With the above example as reference, an example of a decent unfiltered translation from the extreme stress testing is given:
\begin{center}
\begin{tabular}{p{2.5in}}
    \hline
...
\begin{figure}
    \centering
    \includegraphics[width=0.49\linewidth]{figs/overalle.png}
    \includegraphics[width=0.49\linewidth]{figs/overallm.png}
    \caption{Attention maps of two high overall word count translations. Left attention is generated from the extreme test. Right attention is generated from the manageable.}
\end{figure}
...
\begin{figure}
    \centering
    \includegraphics[width=0.49\linewidth]{figs/uniquee.png}
    \includegraphics[width=0.49\linewidth]{figs/uniquem.png}
    \caption{Attention maps of two high unique word count translations. Left attention is generated from the extreme test. Right attention is generated from the manageable.}
\end{figure}
...
\begin{figure}
    \centering
    \includegraphics[width=0.49\linewidth]{figs/building_attention.png}
    \includegraphics[width=0.49\linewidth]{figs/dog_attention.png}
    \caption{Left attention is generated from the first relevant translation about buildings. Right attention is generated from the second relevant translation about dogs.}
\end{figure}
...
The $0\%$ filter also shows translation deterioration in respect to the example length only $2$ examples of the $30203$ remaining results were complexity 2, with the rest being complexity 1. Putting all the results together, the two likely conditions for these errors are likely both complex and long sentences.
...
ChatGPT was only used for the following areas in the \textbf{Introduction} section for proofreading:
\begin{itemize} 
    \item  "MT tools \st{have primarily been utilized to translate}" $\rightarrow$ "MT tools \textit{primarily translated}"
    \item "KyungHyun Cho et al. reveal\st{s}" $\rightarrow$ "KyungHyun Cho et al. reveal\textit{ed}"
    \item italicization of the title for Bahdanau's paper
    \item punctuation.
\end{itemize}
Other sections were not passed into ChatGPT due to the volume of text.
...
\begin{itemize}
    ...
    \item LSTM vs GRU from Medium \cite{Srivatsavaya_2023}
\end{itemize}

The codebase that was used was derived from a tutorial implementaion of the official seq2seq PyTorch modeul \cite{github}. The original model was kept as is with additional datasets and code implementations to stress test the mode. My implementation can be found at https://github.com/PikaSannnnn/CSE256-Literature-Survey-and-Proposal/tree/main.
